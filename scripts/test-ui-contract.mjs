...
function readUi(file) { return fs.readFileSync(path.join(uiRoot, file), "utf8"); }
function readLib(file) { return fs.readFileSync(path.join(libRoot, file), "utf8"); }
function expectSource(source, label, pattern, description) { assertions += 1; if (!pattern.test(source)) errors.push(`${label}: ${description}`); }
function expectUi(file, checks) { const source = readUi(file); for (const [pattern, description] of checks) expectSource(source, file, pattern, description); }
...
expectUi("textarea.tsx", [[/<textarea\b/, "Textarea must remain a native textarea like the shadcn baseline."],[/React\.ComponentProps<"textarea">/, "Textarea public props must remain native-compatible."],[/aria-invalid:border-destructive/, "Textarea must expose invalid-state styling."]]);
expectUi("label.tsx", [[/<label\b/, "Label must remain a native label."],[/React\.ComponentProps<"label">/, "Label public props must remain native-compatible."]]);
expectUi("card.tsx", [[/size\?: "default" \| "sm"/, "Card must preserve the current shadcn size contract."],[/data-slot="card-action"/, "Card must provide CardAction composition."],[/function CardTitle[\s\S]*?<div/, "CardTitle must not force an application heading level."],[/function CardDescription[\s\S]*?<div/, "CardDescription must remain semantically neutral."]]);
expectUi("badge.tsx", [[/@base-ui\/react\/use-render/, "Badge must use Base UI render semantics."],[/class-variance-authority/, "Badge variants must use a typed CVA contract."],[/ghost:/, "Badge must preserve the shadcn ghost variant."],[/link:/, "Badge must preserve the shadcn link variant."],[/slot: "badge"/, "Badge must expose a stable slot state."]]);
