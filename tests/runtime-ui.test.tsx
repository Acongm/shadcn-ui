...
describe('Acongm UI runtime contracts', () => {
  it('keeps Button keyboard reachable and disabled-safe through Base UI', async () => {
    ...
    const onClick = vi.fn();
    render(<div><Button onClick={onClick}>保存</Button><Button disabled onClick={onClick}>不可用</Button></div>);
  ...
  it('binds Label and Input through native form semantics', async () => {
    const user = userEvent.setup();
    render(<div><Label htmlFor="email">邮箱</Label><Input id="email" name="email" /></div>);
  ...
  it('keeps Alert assertive without forcing application heading hierarchy', () => {
    render(<Alert variant="destructive"><AlertTitle>登录失败</AlertTitle><AlertDescription>请检查账号后重试。</AlertDescription></Alert>);
  ...
  it('cycles system → light → dark and persists the shared theme contract', async () => {
    ...
    const toggle = await screen.findByRole('button', { name: /跟随系统，切换到浅色模式/ });
  ...
  it('reacts to operating-system theme changes while system mode is active', async () => {
    ...
    const mediaQuery = {
      ...
      removeEventListener: vi.fn(), dispatchEvent: vi.fn(() => false),
    };
    ...
    act(() => { systemDark = true; changeListener?.(); });
    await waitFor(() => {
      expect(document.documentElement.classList.contains('dark')).toBe(true);
      expect(document.documentElement.dataset.theme).toBe('dark');
      expect(document.documentElement.style.colorScheme).toBe('dark');
